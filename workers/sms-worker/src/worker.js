...
class SmsWorker {
  ...
  async start() {
    ...
    logger.info('SMS worker waiting for messages', { queue: this.queue });
  ...
  async handleMessage(msg) {
    ...
    //
    try {
      await smsWorkerLogic.processSms(payload);
      this.channel.ack(msg);
      //
    } catch (error) {
      const retryCount = Number(payload.retry_count || 0);
      const shouldRetry = smsWorkerLogic.shouldRetry(retryCount);
      if (shouldRetry) {
        payload.retry_count = retryCount + 1;
        await this.channel.sendToQueue(this.queue, Buffer.from(JSON.stringify(payload)), {
          persistent: true,
        });
        logger.info('SMS message requeued', { id: payload.id, retry_count: payload.retry_count });
      } else {
        logger.error('SMS message reached max retry count', { id: payload.id, retry_count });
      }
      this.channel.ack(msg);
    }
  }
